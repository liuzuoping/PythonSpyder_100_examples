...
class XiaoQuBaseSpider(BaseSpider):
    ...
    def get_xiaoqu_info(city, area):
        ...
        for i in range(1, total_page + 1):
            ...
            # xpath = '/html/body/div[4]/div[1]/ul/li[1]/a'
            # l = tree.xpath(xpath)
            # link = l[0].values()[1]
            soup = BeautifulSoup(html, "lxml")
            links = soup.find_all('li', class_="xiaoquListItem")
            for link in links:
                link = link.find('div', class_='title').a['href']
                BaseSpider.random_delay()
                response = requests.get(link, timeout=10, headers=headers)
                html = response.content
                soup = BeautifulSoup(html, "lxml")
                jianzhu = soup.find_all('div', class_="xiaoquInfoItem")
                niandai = jianzhu[0].get_text().strip('建筑年代')
                leixing = jianzhu[1].get_text().strip('建筑类型')
                wuyefei = jianzhu[2].get_text().strip('物业费用')
                # wuyegs = jianzhu[3].get_text().strip('物业公司')
                # kaifashang = jianzhu[4].get_text().strip('开发商')
                loushu = jianzhu[5].get_text().strip('楼栋总数')
                wushu = jianzhu[6].get_text().strip('房屋总数')
            # 获得有小区信息的panel
            house_elems = soup.find_all('li', class_="xiaoquListItem")
            for house_elem in house_elems:
                price = house_elem.find('div', class_="totalPrice")
                # tag=house_elem.find('div', class_="tagList")
                name = house_elem.find('div', class_='title')
                on_sale = house_elem.find('div', class_="xiaoquListItemSellCount")
                # link=house_elem.find('div', class_='title').a['href']
                ...
                # 作为对象保存
                xiaoqu = XiaoQu(chinese_district, chinese_area, name, price,on_sale,link,niandai,leixing,wuyefei,loushu,wushu)
